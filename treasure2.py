...
if "initialized" not in st.session_state:
    ...
    st.session_state.epsilon = 0.10  # default ε
...
def choose_action_eps_greedy(r: int, c: int, epsilon: float) -> str | None:
    """Pick an action using ε-greedy over the current Q-values (valid moves only)."""
    candidates = [a for a in ACTIONS
                  if valid_move(r, c, a, st.session_state.n_rows, st.session_state.n_cols)]
    if not candidates:
        return None
    ...
    # Exploit
    s = state_index(r, c, st.session_state.n_cols)
    idxs = [ACTION_IDX[a] for a in candidates]
    qvals = np.array([st.session_state.Q[s, i] for i in idxs], dtype=float)

    finite_mask = np.isfinite(qvals)
    if not finite_mask.any():
        return np.random.choice(candidates)

    qvals = qvals[finite_mask]
    idxs  = [idxs[i] for i, ok in enumerate(finite_mask) if ok]

    max_q = np.max(qvals)
    best_local = [i for i, q in zip(idxs, qvals) if q == max_q]
    best_idx = np.random.choice(best_local)  # random tie-break among best
    return ACTIONS[best_idx]

def play_eps_greedy_episode(max_steps: int = 500, epsilon: float = 0.1) -> bool:
    """Run one episode using ε-greedy action selection on the current Q."""
    steps = 0
    while steps < max_steps:
        r, c = st.session_state.pos
        action = choose_action_eps_greedy(r, c, epsilon)
        if action is None:
            st.session_state.log.append("No valid moves; episode aborted.")
            return False
        if do_step(action):   # Q-update happens inside do_step
            return True
        steps += 1
...
with left:
    ...
    st.write("---")
    st.subheader("Auto-play (policy)")
    policy = st.radio("Policy for auto episodes:",
                      ["Random (uniform)", "ε-greedy (uses Q)"],
                      index=1,  # default to ε-greedy
                      horizontal=False)
    ...
    if st.button("▶️ Run episodes", use_container_width=True):
        ...
        for _ in range(int(n_eps)):
            if policy.startswith("Random"):
                ...
                found = play_eps_greedy_episode(max_steps=int(max_steps), epsilon=float(st.session_state.epsilon))
            if found:
                wins += 1
        st.info(
            f"Ran {int(n_eps)} episode(s) with **{policy}**. Treasure found in {wins}. "
